...
>Below, we can see the power of using dplyr and the pipeline "%>%" to chain commands together. The following commands, counts the number of different options per brand of Ramen, arranges the brand in descending order (from most to less options) and finally selects only the top10 brands with the most options.

>We can easily plot in increasing order the brands using ggplot

```{r}
most.options <- count(rr, Brand) %>% arrange( desc(n)) %>% top_n(10,n)
c <- ggplot(most.options, aes(x=reorder(Brand, -n), y=n))
