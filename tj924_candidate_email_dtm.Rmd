...
## Data cleaning
This is a suitable place for cleaning data. For example, removal of HTML tags and style declarations from the emails may be appropriate depending on the training strategy.


```{r}
glimpse(email_df)  

email_df$email <- email_df$email %>%
  str_replace_all(.,'[\\n\\s]{2,}','') %>% # Remove more than one common whitespace
  str_replace_all(.,'="[^"]*?"','') %>% # Remove quotes following a =, common to inline meta or style data
  str_replace_all(., '<style>.*?style>','') %>% # Remove style tag content
  str_replace_all(., '</?[^>]+>','') %>% # Remove html tags
  str_replace_all(.,'\\/\\*.*?\\*\\/','') %>% # Remove css comment text
  str_replace_all(.,'[^{]{2,40}\\{[^\\}]*?\\}','') # Remove css declarations
